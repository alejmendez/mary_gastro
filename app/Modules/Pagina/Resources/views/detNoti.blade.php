	...
	<aside class="sidebar">
		...
		<div id="bunch_blog_recent_news-2" class="widget sidebar-widget widget_bunch_blog_recent_news">
			<div class="recent-posts">
				...
				@foreach($noticias->get() as $not)
					<div class="post">
						<figure class="post-thumb">
							<img width="75" height="75" src="{{ url('public/archivos/noticias/' . $not->imagenes->first()->archivo) }}" class="attachment-75x75 size-75x75 wp-post-image" alt="5"  sizes="(max-width: 75px) 100vw, 75px" />
							<a href="{{url('/blog/noticia/'. $not->slug)}}" class="overlay-link">
								<span class="fa fa-link"></span>
							</a>
						</figure>
						<div class="desc-text"><a href="{{url('/blog/noticia/'. $not->slug)}}">{{ $not->titulo }} ...</a></div>
						<div class="time">
							{{ $controller->meses[$not->published_at->month] }}
							{{ $not->published_at->day }},
							{{ $not->published_at->year }}
						</div>
					</div>
				@endforeach
			</div>
